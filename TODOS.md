...
### SAM mesh deploy is broken — every K8s pod would CrashLoopBackOff (DEFERRED — Epic 6 parked, 2026-04-27)

> **2026-04-27 status:** This whole section is parked. The active deployment path is the in-process LangGraph runner (`api-service` + `arq-worker`, single image, zero SAM wiring); the SAM mesh + Layer2Aggregator topology was decommissioned in rev 3 and the 12-pod kustomize base no longer reflects what we ship. Section A of the Solace HA plan never landed — we never applied SAM manifests against a cluster. If/when SAM activation is revived under Epic 6, fix the issues below first; otherwise the failure modes are theoretical against a topology we don't run. Kept here for historical context.
...
- **Why hidden today:** the previous `staging-deploy.yml` (now consolidated into `deploy.yml`) only did `kubectl set image` (image-tag bumps on existing Deployments). No one has applied the `k8s/base/` manifests against a real cluster, so the bomb has not gone off. The actual production code path is `src/pipeline/runner.py` (single-process, "No Solace, no Redis" per its module docstring) — which uses zero SAM wiring. `grep "from solace_agent_mesh\|import solace_agent_mesh" src/ tests/` returns zero hits.
...
- **Depends on:** Resolution before Section A of the Solace HA plan ships (Section A adds `kubectl apply -k` to the staging path of `deploy.yml`, which is what would trigger the crash-loop).
