...
class TestFieldOwnership:
    def test_authorized_write_passes(self):
        ...
        # case_processing is allowed to write domain
        validate_field_ownership("case_processing", original, updated)

    def test_unauthorized_write_raises(self):
        original = CaseState().model_dump()
        updated = CaseState(deliberation={"conclusion": "test"}).model_dump()
        with pytest.raises(FieldOwnershipError, match="case_processing"):
            validate_field_ownership("case_processing", original, updated)
    ...
    def test_audit_log_always_allowed(self):
        ...
        updated = append_audit_entry(updated, agent="case_processing", action="test")
        validate_field_ownership("case_processing", original, updated.model_dump())
