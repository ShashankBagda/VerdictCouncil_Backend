...
def _e(name: str) -> PG_ENUM:
    """Return a pre-existing PostgreSQL enum reference that suppresses DDL."""
...
def upgrade() -> None:
    ...
    op.execute(sa.text("CREATE TYPE factconfidence AS ENUM ('high', 'medium', 'low', 'disputed')"))
    ...
    op.execute(
        sa.text(
            "CREATE TYPE argumentside AS ENUM ('prosecution', 'defense', 'claimant', 'respondent')"
        )
    )
    op.execute(
        sa.text(
