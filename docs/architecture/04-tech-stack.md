...
| **Monitoring** | DO Monitoring + DO Kubernetes Monitoring Stack (1-click) + LangSmith | — | DO provides managed-service / node metrics. Cluster-level Prometheus + Grafana + Alertmanager + node-exporter + kube-state-metrics is installed via the **[DigitalOcean Marketplace 1-click app](https://marketplace.digitalocean.com/apps/kubernetes-monitoring-stack)** (`kubernetes-monitoring-stack`), which scrapes `/metrics` on `api-service` and the worker. LangSmith captures graph-level LLM traces (separate concern) |
